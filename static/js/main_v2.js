document.addEventListener("DOMContentLoaded", () => {
  ...
  if (headerNavMobile) {
    const closeAllSubmenus = (excludeItem = null) => {
      const openMenus = headerNavMobile.querySelectorAll(".has-sub-menu.active");
      openMenus.forEach((menu) => {
        if (excludeItem && menu === excludeItem) return;
    ...
    window.addEventListener("resize", () => {
      if (window.innerWidth >= 992) closeAllSubmenus();
    });
    ...
    document.addEventListener("click", (event) => {
      ...
      if (!headerNavMobile.contains(target)) closeAllSubmenus();
    });
  }
...
const BASE = "https://cms.apparelbrandingcompany.in";
...
async function loadServices() {
  ...
  const res = await fetch(
    `${BASE}/api/products?populate[images][populate]=image&sort=createdAt:desc&_=${Date.now()}`,
    { cache: "no-store" }
  );
  ...
  try {
    ...
    // ✅ NEW STRUCTURE (images → image → url)
    if (p.images && p.images.length > 0) {
      const firstColor = p.images[0];

      if (firstColor.image && firstColor.image.length > 0) {
        imgRaw = firstColor.image[0].url;
      }
    }

    // ✅ OLD STRUCTURE (image → url)
    if (!imgRaw && p.image && p.image.length > 0) {
      imgRaw = p.image[0].url;
    }
    ...
    container.innerHTML += `
      <div class="col-md-4 col-sm-6 col-12">
        <div class="ul-service-card p-4 bg-white rounded shadow-sm">

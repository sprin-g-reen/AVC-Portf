...
def _normalize_footer_columns(value):
    def _fallback_footer_url(title, label, current_url):
        url = str(current_url or "").strip()
        title_l = str(title or "").strip().lower()
        if title_l in {"our products", "our product", "products"}:
            return "/shop"
        if url and url != "#":
            return url
        safe_category = quote(str(label or "").strip())
        if not safe_category:
            return url or "#"
        if title_l in {"white papers", "white paper"}:
            return f"/shop?category={safe_category}"
        return url or "#"
    ...
    for item in value:
        ...
        for link in links:
            link["url"] = _fallback_footer_url(title, link.get("label"), link.get("url"))
        if title and links:
            ...
            query = urlencode(
                {
                    ...
                    "populate[nav_links][populate]": "*",
                    "populate[navbar_links][populate]": "*",
                    "populate[footer_columns][populate]": "*",
                    "populate[footer_links][populate]": "*",
                    "pagination[page]": 1,
...
>>>>>>> f3025387d8e2326436640e00906cf1ef18ad292d
            ...
            for index, row in enumerate(data, start=1):
                ...
                raw_strapi_id = str(_safe_get(row, "id", default=f"{page}-{index}"))
                external_id = str(product.get("external_id", "")).strip()
                product_key = external_id if external_id and external_id not in ("None", "") else raw_strapi_id
